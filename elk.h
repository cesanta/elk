...
jsval_t js_eval(struct js *, const char *buf, size_t len);  // Execute JS code
jsval_t js_glob(struct js *);   // Return global object
jsval_t js_mkobj(struct js *);  // Create a new object
jsval_t js_import(struct js *, uintptr_t func, const char *signature);
void js_set(struct js *, jsval_t obj, const char *key, jsval_t val);
int js_usage(struct js *);  // Return memory usage percentage
